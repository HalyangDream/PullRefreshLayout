...
import com.example.pullrefreshlayout.LoadingImage;
...
/**
 * 转载自PullableRefreshLayout 只是稍作修改
...
public class PullToRefreshLayout extends FrameLayout {
    ...
    // 上拉的箭头
    private LoadingImage pullUpView;
    // 正在加载的图标
...
//	private View loadStateImageView;
    ...
    private int paddingTop = 0; //默认30 项目需要
     ...
     * 执行自动回滚的handler
     */
    private PullHanlder updateHanlder;
    ...
    private void initView(Context context) {
        mContext = context;
        updateHanlder = new PullHanlder(this);
        timer = new MyTimer(updateHanlder);
...
//		refreshingAnimation.setInterpolator(lir);
    ...
    private void hide() {
//        if (pullView != null) {
//            pullView.stopLoading();
//        }
//        if (pullUpView != null) {
//            pullUpView.stopLoading();
//        }
...
//					.setBackgroundResource(R.drawable.refresh_open);
                break;
            case FAIL:
            default:
                // 刷新失败
...
//					.setBackgroundResource(R.drawable.refresh_open);
        ...
        if (pullDownY > 0) {
            // 刷新结果停留1秒
            updateHanlder.sendEmptyMessageDelayed(0, 500);
        } else {
            updateHanlder.obtainMessage(0);
        }
    }
...
//			loadStateImageView.setVisibility(View.GONE);
        ...
        if (pullUpY < 0) {
            ...
            updateHanlder.sendEmptyMessage(0);
        }
    }
...
//			loadingView.setVisibility(View.GONE);
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        switch (ev.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (mEvents == 0) {
                    if (pullDownY > 0 || (((Pullable) pullableView) != null && ((Pullable) pullableView).canPullDown() && canPullDown && state != LOADING)) {
                        // 可以下拉，正在加载时不能下拉
                        // 对实际滑动距离做缩小，造成用力拉的感觉
    ...
    private void initView() {
        ...
        pullView = (LoadingImage) refreshView.findViewById(R.id.pull_icon);
        refreshStateTextView = (TextView) refreshView
                .findViewById(R.id.state_tv);
//		refreshingView = refreshView.findViewById(R.id.refreshing_icon);
//		refreshStateImageView = refreshView.findViewById(R.id.state_iv);
        // 初始化上拉布局
        pullUpView = (LoadingImage) loadmoreView.findViewById(R.id.pullup_icon);
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        pullableView.layout(0, (int) (pullDownY + pullUpY) + paddingTop,
                pullableView.getMeasuredWidth(), (int) (pullDownY + pullUpY)
                        + pullableView.getMeasuredHeight());
        loadmoreView.layout(0,
                (int) (pullDownY + pullUpY) + pullableView.getMeasuredHeight() + paddingTop,
...
//                pullUpView.stopLoading();
    ...
    private static class PullHanlder extends Handler {
        WeakReference<PullToRefreshLayout> weakReference;

        private PullHanlder(PullToRefreshLayout pullToRefreshLayout) {
            weakReference = new WeakReference<PullToRefreshLayout>(pullToRefreshLayout);
        }

        @Override
        public void handleMessage(Message msg) {
            PullToRefreshLayout pullToRefreshLayout = weakReference.get();
            switch (msg.what) {
                case 0:
                    if (pullToRefreshLayout != null) {
                        pullToRefreshLayout.changeState(DONE);
