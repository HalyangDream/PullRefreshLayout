...
/**
 * 实现继承Google 官方的下拉刷新控件实现上拉加载功能
 * 借鉴 PullToRefreshLayout 实现
 * Create HalyangDream
 */
public class SwipeRefreshView extends SwipeRefreshLayout {
    ...
    private boolean onListViewLoadMore(ListView listView) {
        ...
        if (listView.getCount() == 0) {
            ...
            int firstVisiable = listView.getFirstVisiblePosition();
            if (getChildAt(lastVisible - firstVisiable) != null
                    && getChildAt(lastVisible - firstVisiable).getBottom() <= getMeasuredHeight()) {
                return true;
            }
        }
        return false;
    ...
    private boolean onRecyclerViewLoadMore(RecyclerView recyclerView) {
...
//            Log.i("PullableRecyclerView", "因为没有数据控件到底了-->001-->" + adapter.getItemCount());
        ...
        if (layoutManager instanceof GridLayoutManager) {
            ...
            final int lastVisiblePosition = ((GridLayoutManager) layoutManager).
                    findLastVisibleItemPosition();
            Log.i(TAG, "lastItemPosition-->" + lastItemPosition + "lastVisiblePosition-->" + lastVisiblePosition);
            if (lastVisiblePosition >= lastItemPosition - 1) {
                ...
                if (lastVisibleChild != null) {
                    Log.i(TAG, "因为数据控件到底了-->002");
                    return lastVisibleChild.getBottom() <= getBottom();
                }
            }
        } else if (layoutManager instanceof LinearLayoutManager) {
            final int lastItemPosition = adapter.getItemCount() - 1;
            final int lastVisiblePosition = ((LinearLayoutManager) layoutManager).
                    findLastVisibleItemPosition();
            Log.i(TAG, "lastItemPosition-->" + lastItemPosition + "lastVisiblePosition-->" + lastVisiblePosition);
            if (lastVisiblePosition >= lastItemPosition - 1) {
                final int childIndex = lastVisiblePosition -
            ...
            Log.i(TAG, "lastItemPosition-->" + lastItemPosition + "lastVisiblePosition-->" + lastItems);
